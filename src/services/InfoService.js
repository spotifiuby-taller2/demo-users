...
class InfoService {
  defineEvents(app) {
     ...
     */
    app.post(constants.USERS_CREATE_ADMIN_URL,
     ...
     */
    app.post(constants.USERS_VERIFIED_URL,
      this.verifiedUser.bind(this));
     ...
     */
    app.post(constants.USERS_UNVERIFIED_URL,
      this.unverifiedUser.bind(this));
  ...
  async addFavArtist(req, res) {
    ...
    utils.setBodyResponse({status: 'Nuevo favorito agregado'}, 200, res);
  }
  ...
  async getNotificationList(req, res) {
    ...
    const emissorId = req.query.idEmissor;
    const emissor = await Users.findOne(
      {
        where: {[Op.and]: [{id: emissorId}, {isBlocked: false}]}
      }
    ).catch(err => ({error: err.toString()}));
